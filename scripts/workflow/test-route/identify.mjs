...
export default async function identify({ github, context, core }, body, number, sender) {
    ...
    if (isPR) {
        ...
        if (
            issue.labels.some((e) => e.name === criticalFailure) ||
            issue.labels.some((e) => e.name === routeTestFailed)
        ) {
        ...
        if (isPR) {
            const headingIndex = ast.children.findIndex(
                (node) =>
                    node.type === "heading" &&
                    node.children?.some(
                        (child) => child.type === "text" && child.value.trim() === requiredHeading,
                    ),
            );
            ...
            if (headingIndex === -1) {
                ...
                const nextHeading = ast.children.findIndex(
                    (node, i) => i > headingIndex && node.type === "heading",
                );
                if (nextHeading !== -1) {
