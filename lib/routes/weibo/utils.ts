...
const getDescriptionRenderParams = (routeParams, params = {}) => ({
    showEmojiInDescription: fallback(
        params.showEmojiInDescription,
        queryToInteger(routeParams.showEmojiInDescription),
        false,
    ),
    showLinkIconInDescription: fallback(
        params.showLinkIconInDescription,
        queryToInteger(routeParams.showLinkIconInDescription),
        true,
    ),
});
...
const weiboUtils = {
    ...
    tryWithCookies: (() => {
        ...
        const verifier = (resp: any): void => {
            if (resp?.data?.ok === -100) {
                throw new RenewWeiboCookiesError(
                    `Cookies expired. Msg: ${resp?.data?.msg || ""} ${resp?.data?.url || ""}`,
                );
            }
        };
        return async <T>(
            callback: (cookies: string, verifier: (resp: any) => void) => Promise<T>,
        ): Promise<T> => {
            try {
                ...
                if (errors > 10) {
                    logger.warn(
                        `Too many errors while fetching data from weibo API, renewing Cookies: ${error.message}`,
                    );
                    logger.info(
                        "Please open an issue on GitHub if renewing Cookies fixes the error",
                    );
                } else if (
                    (error.name === "HTTPError" || error.name === "FetchError") &&
                    error.status === 432
                ) {
                    // empty
    ...
    formatExtended: (ctx, status, uid, params = {}, picsPrefixes = []) => {
        ...
        const mergedParams = {
            ...
            authorNameBold: fallback(
                params.authorNameBold,
                queryToBoolean(routeParams.authorNameBold),
                false,
            ),
            showAuthorInTitle: fallback(
                params.showAuthorInTitle,
                queryToBoolean(routeParams.showAuthorInTitle),
                false,
            ),
            showAuthorInDesc: fallback(
                params.showAuthorInDesc,
                queryToBoolean(routeParams.showAuthorInDesc),
                false,
            ),
            showAuthorAvatarInDesc: fallback(
                params.showAuthorAvatarInDesc,
                queryToBoolean(routeParams.showAuthorAvatarInDesc),
                false,
            ),
            showAtBeforeAuthor: fallback(params.showAtBeforeAuthor, null, false),
            showEmojiForRetweet: fallback(
                params.showEmojiForRetweet,
                queryToBoolean(routeParams.showEmojiForRetweet),
                false,
            ),
            showRetweetTextInTitle: fallback(
                params.showRetweetTextInTitle,
                queryToBoolean(routeParams.showRetweetTextInTitle),
                true,
            ),
            addLinkForPics: fallback(
                params.addLinkForPics,
                queryToBoolean(routeParams.addLinkForPics),
                false,
            ),
            showTimestampInDescription: fallback(
                params.showTimestampInDescription,
            ...
            heightOfPics: fallback(
                params.heightOfPics,
                queryToInteger(routeParams.heightOfPics),
                -1,
            ),
            sizeOfAuthorAvatar: fallback(
                params.sizeOfAuthorAvatar,
                queryToInteger(routeParams.sizeOfAuthorAvatar),
                48,
            ),
            ...
            preferMobileLink: fallback(
                params.preferMobileLink,
                queryToBoolean(routeParams.preferMobileLink),
                false,
            ),
        };
        ...
        let retweeted = "";
        // 长文章的处理
        let htmlNewLineUnreplaced =
            (status.longText && status.longText.longTextContent) || status.text || "";
        htmlNewLineUnreplaced = formatDescriptionText(
            htmlNewLineUnreplaced,
            descriptionRenderParams,
        );

        // 提取 话题作为 category
        const category: string[] = htmlNewLineUnreplaced
        ...
        // 处理图片的链接
        htmlNewLineUnreplaced = htmlNewLineUnreplaced.replaceAll(
            /<a\s+href="https?:\/\/[^"]+\.(jpg|png|gif)"/g,
            (match) => `${match} data-rsshub-image="href"`,
        );
        ...
        if (showAuthorInDesc) {
            ...
            usernameAndAvatar += authorNameBold
                ? `<strong>${name}</strong></a>:&ensp;`
                : `${name}</a>:&ensp;`;
        ...
        // drop live photo
        const livePhotoCount = status.pics
            ? status.pics.filter((pic) => pic.type === "livephoto").length
            : 0;
        ...
        if (status.retweeted_status) {
            ...
            retweeted += weiboUtils.formatExtended(
                ctx,
    ...
    formatVideo: (itemDesc, status) => {
        ...
        const livePhotos =
            status.pics && status.pics.filter((pic) => pic.type === "livephoto" && pic.videoSrc);
        ...
        if (pageInfo && pageInfo.type === "video") {
            ...
            const videoHd =
                urls.mp4_hd_mp4 || mediaInfo.mp4_hd_url || mediaInfo.stream_url_hd || "";
    ...
    formatArticle: async (ctx, itemDesc, status) => {
        ...
        if (pageInfo && pageInfo.type === "article" && pageInfo.page_url) {
            ...
            if (article && article.title && article.content) {
                ...
                let articleMeta =
                    '<p style="line-height: 1.66; color: #999;margin: 0 0 0.75rem;font-size: 0.75rem;padding: 0">';
                if (isArticleNonFree) {
                ...
                html +=
                    '<div style="height: 0;border-bottom: 1px dashed #999;margin-bottom: 0.75rem;"></div>'; // 分割线
                ...
                $("img").each((_, elem) => {
                    ...
                    let style = elem.attr("style") || "";
                    style =
                        "display: block;max-width: 100%;margin-left: auto;margin-right: auto;min-width: 50px;" +
                        style;
                    elem.attr("style", style);
                });
        ...
        if (status && status.comments_count && status.id && status.mid) {
            ...
            if (response.data && response.data.data) {
                ...
                for (const comment of comments) {
                    ...
                    const commentText = formatDescriptionText(
                        comment.text,
                        descriptionRenderParams,
                    );
                    ...
                    if (comment.comments) {
                        ...
                        for (const com of comment.comments) {
                            // 评论的带有图片的评论直接输出图片
                            const pattern =
                                /<a\s+href="https:\/\/weibo\.cn\/sinaurl\?u=([^"]+)"[^>]*><span class='url-icon'><img[^>]*><\/span><span class="surl-text">(查看图片|评论配图|查看动图)<\/span><\/a>/g;
                            let replyText = com.text;
                            const matches = replyText.match(pattern);
                            if (matches) {
                                for (const match of matches) {
                                    const hrefMatch = match.match(
                                        /href="https:\/\/weibo\.cn\/sinaurl\?u=([^"]+)"/,
                                    );
                                    if (hrefMatch) {
                                        // 获取并解码 href 中的图片 URL
