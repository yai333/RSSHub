...
const renderVideo = (video, poster) =>
    `<video controls poster="${poster}"><source src="${video}" type="video/mp4"></video>`;
...
async function handler(ctx) {
    ...
    const INSTALKER_BACK_API_KEY =
        "1263fd960207e2d481eff2c60feeae1541f6e90419283f7e674a36d8ac706462";
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
        ...
        const allSuccess =
            Array.isArray(status) && status.every((item) => item.status === "success");
        if (allSuccess) {
            break;
        }
        if (attempt < 9) {
    ...
    const items = data.map((item) => ({
        ...
        guid:
            item.id ??
            h64ToString(`${item.account_name}:${item.takenAt}:${item.thumbnailImageUrl}`),
