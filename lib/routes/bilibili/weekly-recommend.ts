...
async function handler(ctx) {
    ...
    return {
        ...
        item: await Promise.all(
            data.map(async (item) => {
                const subtitles =
                    isJsonFeed && !config.bilibili.excludeSubtitles && item.bvid
                        ? await cache.getVideoSubtitleAttachment(item.bvid)
                        : [];
                return {
                    title: item.title,
                    description: utils.renderUGCDescription(
                        embed,
                        item.cover,
                        `${weekly_name} ${item.title} - ${item.rcmd_reason}`,
                        item.param,
                        undefined,
                        item.bvid,
                    ),
                    link:
                        weekly_number > 60 && item.bvid
                            ? `https://www.bilibili.com/video/${item.bvid}`
                            : `https://www.bilibili.com/video/av${item.param}`,
