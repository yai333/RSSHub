                ...
                <>
                    ...
                    <ul>
                        {result.summary.map((summary, index) => (
                            <li key={`${summary.description ?? 'summary'}-${index}`}>{summary.description}</li>
                        ))}
                    </ul>
                </>
            ) : null}
                ...
                <>
                    {result.related_content.galleries?.flatMap((gallery, galleryIndex) =>
                        (gallery.content_elements ?? []).map((element, elementIndex) =>
                            element.type === 'image' ? (
                                <figure key={`gallery-${galleryIndex}-image-${elementIndex}`}>
                    ...
                    {result.related_content.galleries?.flatMap((gallery, galleryIndex) =>
                        (gallery.content_elements ?? []).map((element, elementIndex) =>
                            element.type === 'video' ? (
                                <figure key={`gallery-${galleryIndex}-video-${elementIndex}`}>
                                    <video controls poster={element.thumbnail?.url}>
                                        <source src={element.source?.mp4} type="video/mp4" />
                                    </video>
                                    {element.description}
                                </figure>
                            ) : null
