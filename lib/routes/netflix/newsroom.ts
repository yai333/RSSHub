...
const renderImage = ({ url, alt, caption }: { url: string; alt?: string; caption?: string }) =>
    `<figure><img src="${url}" alt="${alt || ""}">${caption ? `<figcaption>${caption}</figcaption>` : ""}</figure>`;
const renderVideo = ({
    url,
    poster,
    contentType,
...
const render = (node) => {
    ...
    switch (node.nodeType) {
        ...
        case "embedded-entry-inline": {
            ...
            if (
                embedLink.startsWith("https://www.youtube.com/") ||
                embedLink.startsWith("https://youtu.be/")
            ) {
                const youtubeId =
                    node.data.embedLink.split("youtube.com/watch?v=")[1]?.split("&")[0] ||
                    node.data.embedLink.split("youtu.be/")[1]?.split("?")[0];
...
async function handler(ctx) {
    ...
    const list = [
        ...response.entities.region.regionArticles,
        ...response.entities.global.globalArticles,
    ].map((i) => ({
        title: i.title,
        ...
        category: [
            ...new Set([
                ...i.categories.map((category) => category.label),
                ...i.locations.map((location) => location.label),
            ]),
        ],
    ...
    const items = await Promise.all(
        list.map((item) =>
            cache.tryGet(item.link, async () => {
                ...
                if (
                    slug.heroVideoUrl &&
                    !slug.heroVideoUrl.startsWith("https://www.youtube.com/")
                ) {
                    item.description += renderVideo({
                        url: slug.heroVideoUrl,
                        poster: slug.heroImage?.url
                            ? `https:${slug.heroImage.url.replace("?w=2560", "")}`
                            : "",
                        contentType: "video/mp4",
                    });
                } else if (slug.youtubeHero) {
                    item.description += renderYoutube(
                        slug.youtubeHero.videoLink.split("youtube.com/watch?v=")[1],
                    );
