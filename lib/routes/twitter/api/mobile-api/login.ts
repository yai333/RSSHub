...
const headers = {
    "User-Agent":
        "TwitterAndroid/10.21.0-release.0 (310210000-r-0) ONEPLUS+A3010/9 (OnePlus;ONEPLUS+A3010;OnePlus;OnePlus3;0;;1;2016)",
            ...
            try {
                ...
                const runTask = async ({ data }) => {
                    ...
                    if (!(subtask_id in flowTasks)) {
                        logger.error(
                            `Twitter login flow task failed: unknown subtask: ${subtask_id}`,
                        );
                        return;
                    }
                ...
                if (authentication) {
                    ...
                    logger.error(
                        `Twitter login failed. ${JSON.stringify(task.data?.subtasks, null, 2)}`,
                    );
                }
