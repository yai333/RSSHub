...
async function handler(ctx) {
    ...
    const header = serverData.shelves.find((item) => item.contentType === "showHeaderRegular")
        .items[0];

    const bearerToken = await cache.tryGet(
        "apple:podcast:bearer",
        async () => {
            const moduleAddress = new URL($('head script[type="module"]').attr("src"), baseUrl)
                .href;
    ...
    const episodeReponse = await ofetch(
        `https://amp-api.podcasts.apple.com/v1/catalog/us/podcasts/${numericId}/episodes`,
        {
            query: {
                "extend[podcast-channels]":
                    "editorialArtwork,subscriptionArtwork,subscriptionOffers",
                include: "channel",
                limit: 25,
                with: "entitlements",
                l: "en-US",
    ...
    const episodes = episodeReponse.data.map(({ attributes: item }) => {
        ...
        return {
            ...
            itunes_item_image: item.artwork.url.replace(
                /\{w\}x\{h\}(?:\{c\}|bb)\.\{f\}/,
                "3000x3000bb.webp",
            ),
    ...
    const channel = episodeReponse.data
        .find((d) => d.type === "podcast-episodes")
        ?.relationships?.channel?.data?.find((d) => d.type === "podcast-channels")?.attributes;
    ...
    return {
        ...
        image: (
            (channel?.logoArtwork || channel?.subscriptionArtwork)?.url ||
            header.contextAction.podcastOffer.artwork.template
        ).replace(/\{w\}x\{h\}(?:\{c\}|bb)\.\{f\}/, "3000x3000bb.webp"),
        itunes_category:
            header.metadata.find((d) => Object.hasOwn(d, "category")).category?.title ||
            header.metadata.find((d) => Object.hasOwn(d, "category")).category,
    };
}
