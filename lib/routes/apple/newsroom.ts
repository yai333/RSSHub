...
const articleCleanupSelector =
    ".scrollfade, .article-header, .featured-header, .paddlenav, .articleshare, .nr-article-share, .docsanddownloads, .presscontacts, .sharesheet, .nr-accordion, .sosumi";
    ...
    cache.tryGet(item.link, async () => {
        ...
        return {
            ...
            pubDate: pubDate
                ? parseDate(pubDate)
                : fallbackDate
                  ? parseDate(fallbackDate, "YYYY 年 M 月 D 日")
                  : undefined,
...
async function handler(ctx) {
    ...
    return {
        ...
        feedLink: feedUrl,
        description:
            "Apple 新闻中心是 Apple 新闻的来源。阅读新闻稿、获取最新消息、观看视频和下载图片。",
        item: items,
        language: "zh-CN",
    };
}
