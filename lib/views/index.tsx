...
const Index: FC<{ debugQuery: string | undefined }> = ({ debugQuery }) => {
    ...
    const showDebug =
        !config.debugInfo || config.debugInfo === "false"
            ? false
            : config.debugInfo === "true" || config.debugInfo === debugQuery;
    ...
    const info = {
        ...
        debug: [
            ...
            },
            {
                ...
                value: debug.request
                    ? ((debug.hitCache / debug.request) * 100).toFixed(2) + "%"
                    : 0,
            },
            {
            ...
            },
            {
                name: "Health",
                value: debug.request
                    ? ((1 - debug.error / debug.request) * 100).toFixed(2) + "%"
                    : 0,
            },
            {
                name: "Uptime",
        ...
        <Layout>
            ...
            <div className="w-full grow shrink-0 py-8 flex items-center justify-center flex-col space-y-4">
                ...
                <p className="max-w-xl text-center text-zinc-500">
                    ...
                    , an AI RSS reader built for feed discovery and modern reading workflows. Source
                ...
                <div className="font-bold space-x-4 text-sm">
                    <a target="_blank" href="https://docs.rsshub.app">
                        <button className="text-white bg-[#F5712C] hover:bg-[#DD4A15] py-2 px-4 rounded-full transition-colors">
                            Home
                        </button>
                    </a>
                    <a target="_blank" href="https://folo.is/">
                        <button className="bg-zinc-200 hover:bg-zinc-300 py-2 px-4 rounded-full transition-colors">
                            Try Folo
                        </button>
                    </a>
                    ...
                    <details className="text-xs w-96 !mt-8 max-h-[400px] overflow-auto">
                        ...
                        {info.debug.map((item) => (
                            <div class="debug-item my-3 pl-8">
                                <span class="debug-key w-32 text-right inline-block mr-2">
                                    {item.name}:{" "}
                                </span>
                                <span class="debug-value inline-block break-all align-top">
                                    {item.value}
                                </span>
                            </div>
                        ))}
