...
const middleware: MiddlewareHandler = async (ctx, next) => {
    ...
    if (data) {
        ...
        if (ctx.req.query("sorted") !== "false") {
            data.item = data.item.toSorted(
                (a: DataItem, b: DataItem) => +new Date(b.pubDate || 0) - +new Date(a.pubDate || 0),
            );
        }
            ...
            data.item = data.item.filter((item) => {
                ...
                const isFilter =
                    regex instanceof RE2JS
                        ? regex.matcher(title).find() ||
                          regex.matcher(description).find() ||
                          regex.matcher(author).find() ||
                          category.some((c) => regex.matcher(c).find())
                        : title.match(regex) ||
                          description.match(regex) ||
                          author.match(regex) ||
                          category.some((c) => c.match(regex));
        ...
        if (
            ...
            (ctx.req.query("filter_title") ||
                ctx.req.query("filter_description") ||
                ctx.req.query("filter_author") ||
                ctx.req.query("filter_category"))
        ) {
            data.item = data.item.filter((item) => {
                const title = item.title || "";
                const description = item.description || title;
                ...
                if (ctx.req.query("filter_title")) {
                    ...
                    isFilter =
                        titleRegex instanceof RE2JS
                            ? titleRegex.matcher(title).find()
                ...
                if (ctx.req.query("filter_description")) {
                    ...
                    isFilter =
                        isFilter &&
                        (descriptionRegex instanceof RE2JS
                            ? descriptionRegex.matcher(description).find()
                ...
                if (ctx.req.query("filter_author")) {
                    ...
                    isFilter =
                        isFilter &&
                        (authorRegex instanceof RE2JS
                            ? authorRegex.matcher(author).find()
                            : !!authorRegex.test(author));
                }
                if (ctx.req.query("filter_category")) {
                    ...
                    isFilter =
                        isFilter &&
                        category.some((c) =>
                            categoryRegex instanceof RE2JS
                                ? categoryRegex.matcher(c).find()
                                : c.match(categoryRegex),
                        );
                }
        ...
        if (
            ctx.req.query("filterout") ||
            ctx.req.query("filterout_title") ||
            ctx.req.query("filterout_description") ||
            ctx.req.query("filterout_author") ||
            ctx.req.query("filterout_category")
        ) {
            data.item = data.item.filter((item) => {
                const title = item.title;
                const description = item.description || title;
                ...
                if (ctx.req.query("filterout") || ctx.req.query("filterout_title")) {
                    const titleRegex = makeRegex(
                        ctx.req.query("filterout_title") || ctx.req.query("filterout")!,
                    );
                    isFilter =
                        titleRegex instanceof RE2JS
                            ? !titleRegex.matcher(title).find()
                ...
                if (ctx.req.query("filterout") || ctx.req.query("filterout_description")) {
                    const descriptionRegex = makeRegex(
                        ctx.req.query("filterout_description") || ctx.req.query("filterout")!,
                    );
                    isFilter =
                        isFilter &&
                        (descriptionRegex instanceof RE2JS
        ...
        if (ctx.req.query("filter_time")) {
            ...
            data.item = data.item.filter(({ pubDate }) => {
                ...
                try {
                    isFilter =
                        !pubDate ||
                        now - new Date(pubDate).getTime() <=
                            Number.parseInt(ctx.req.query("filter_time")!) * 1000;
                } catch {
                    // no-empty
                }
                return isFilter;
        ...
        if (ctx.req.query("mode")?.toLowerCase() === "fulltext") {
            const tasks = data.item.map(async (item) => {
                ...
                item.description =
                    parsed_result && parsed_result.content.length > 40
                        ? entities.decodeXML(parsed_result.content)
                        : description;
            });
                    ...
                    try {
                        ...
                        if (config.openai.inputOption === "description" && item.description) {
                            const description = await cache.tryGet(
                                `openai:description:${item.link}`,
                                async () => {
                                    const description = convert(item.description!);
                                    const descriptionMd = await getAiCompletion(
                                        config.openai.promptDescription,
                                        description,
                                    );
        ...
        if (ctx.req.query("scihub")) {
            data.item.map((item) => {
                item.link = item.doi
                    ? `${config.scihub.host}${item.doi}`
                    : `${config.scihub.host}${item.link}`;
                return item;
            });
        }
        ...
        if (ctx.req.query("opencc")) {
            for (const item of data.item) {
                ...
                item.description = simplecc(
                    item.description ?? item.title ?? item.link,
                    ctx.req.query("opencc")!,
                );
            }
        }
